...
impl Nibbles {
    ...
    pub unsafe fn pack_to_unchecked(&self, ptr: *mut u8) {
        let slice = slice::from_raw_parts_mut(ptr.cast(), (self.len() + 1) / 2);
    ...
    pub fn extend_from_slice(&mut self, other: &[u8]) {
        ...
        let mut val = U256::from_be_slice(other);
        if len_bytes > 0 {
            val = val.wrapping_shl((32 - len_bytes) * 8);
        }
        self.nibbles |= val.wrapping_shr(self.bit_len());
